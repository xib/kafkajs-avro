...
describe("kafkajs-avro 2", () => {
  ...
  beforeEach(async () => {
    ...
    kafka = new KafkaAvro({
      ...
      avro: {
        url: "http://localhost:8081",
        parseOptions: { wrapUnions: "always" }
      },
      logLevel: logLevel.ERROR
    });
  ...
  it("can produce and consume multiple (parallel) avro encoded messages", async () => {
    const value = {
      customer: {
        name: "Test Testsson",
        address: `Test Avenue ${uuid()}`
...
describe("kafkajs-avro 3", () => {
  ...
  it("can produce and consume multiple (sequencial) avro encoded messages", async () => {
    const value = {
      customer: {
        name: "Test Testsson",
        address: `Test Avenue ${uuid()}`
